...
class UserDAO
{
	...
	public function insertStudent($username,$password,$key,$role,$regchannel,$profilepic,$isverified){
		...
		if(count($resultSet)>0){
			...
			$arr=array('email_id'=>$username,'user_password'=>md5($password),'tbl_role_role_id'=>$role,'verificationkey'=>$key,'usersubkey'=>$usersubkey,'gensubkey'=>$gensubkey,"regchannel"=>$regchannel,"profile_picture"=>$profilepic,"isverified"=>$isverified);
	...
	public function checkLoginSocial($username){
		$resultSet = $this->tableGateway->select(array('email_id' => $username,'isverified'=>'1'));
